...
use Volcano\Cache\Repository as CacheRepository;
use Volcano\Events\Dispatcher;
use Volcano\Queue\Failed\FailedJobProviderInterface;
...
class Worker
{
     ...
     *
     * @var \Volcano\Queue\Failed\FailedJobProviderInterface
     */
    protected $failer;
     ...
     *
     * @var \Volcano\Foundation\Exceptions\Handler
     */
     ...
     * @param  \Volcano\Queue\Contracts\FailedJobProviderInterface  $failer
    ...
    protected function daemonShouldRun($connection, $queue)
    {
        ...
        $result = $this->events->until('Volcano.queue.looping', array($connection, $queue));
     ...
     *
     * @param  \Volcano\Queue\Queue  $connection
     * @param  string  $queue
     * @return \Volcano\Queue\Job|null
     */
     ...
     * @param  string  $connection
     * @param  \Volcano\Queue\WorkerOptions  $options
     * @return void
     */
    ...
    protected function raiseBeforeJobEvent($connection, Job $job)
    {
        if (isset($this->events)) {
            $this->events->dispatch('volcano.queue.processing', array($connection, $job));
        }
    }
    ...
    {
        if (isset($this->events)) {
            $this->events->dispatch('volcano.queue.processed', array($connection, $job));
        }
    }
    ...
    {
        if (isset($this->events)) {
            $this->events->dispatch('volcano.queue.failed', array($connection, $job));
        }
    }
    ...
    {
        $this->events->dispatch('volcano.queue.stopping');
    ...
    {
        if (isset($this->cache)) {
            return $this->cache->get('volcano:queue:restart');
        }
    }
     ...
     *
     * @param  \Volcano\Exception\Handler  $handler
     * @return void
     */
     ...
     *
     * @return \Volcano\Queue\QueueManager
     */
