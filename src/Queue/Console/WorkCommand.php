...
class WorkCommand extends Command
{
     ...
     *
     * @param  \Volcano\Queue\Worker  $worker
     * @return void
     */
    ...
    protected function listenForEvents()
    {
        ...
        $events->listen('volcano.queue.processing', function ($connection, $job)
        {
            $this->writeOutput($job, 'starting');
        });

        $events->listen('volcano.queue.processed', function ($connection, $job)
        {
            $this->writeOutput($job, 'success');
        });

        $events->listen('volcano.queue.failed', function ($connection, $job)
        {
    ...
    protected function runWorker($connection, $queue, $delay, $memory, $daemon = false)
    {
        $this->worker->setDaemonExceptionHandler(
            $this->container['Volcano\Foundation\Exceptions\HandlerInterface']
        );
